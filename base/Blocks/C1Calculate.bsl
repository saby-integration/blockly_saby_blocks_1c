...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ЧастиИмени = Saby_СтрРазделить82(Имя, ".");
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	ЧастиИмени = Saby_СтрРазделить82(ИмяПроцедуры, ".");
...
// Функция c1_calculate_calc_value
//
// Параметры:
// Алгоритм - Строка - Алгоритм
// АргументыВызова - Строка - Аргументы вызова
// Параметры - Структура - Параметры алгоритма
// ПараметрыАлгоритмаМассив - Массив - Массив параметров алгоритма
//
// Возвращаемое значение:
...
Функция c1_calculate_calc_value(Алгоритм, АргументыВызова = Неопределено, 
								Параметры = Неопределено, ПараметрыАлгоритмаМассив = Неопределено)	
	Попытка 
		ОбщийМодульОбщегоНазначения = ОбщийМодульЗначение("ОбщегоНазначения");
		ОбщийМодульОбщегоНазначения.ВычислитьВБезопасномРежиме("2 = 2");
		ВозможноВычислитьВБезопасномРежиме = Истина;
	Исключение
		ВозможноВычислитьВБезопасномРежиме = Ложь;	
	КонецПопытки;
	Если ВозможноВычислитьВБезопасномРежиме Тогда
		...
		Иначе	
			...
			Если ЭтоФункцияПроцедура Тогда
				...
				Исключение
					Результат = ОбщийМодульОбщегоНазначения.ВычислитьВБезопасномРежиме(Алгоритм + "(" + АргументыВызова + ")", Параметры);
				КонецПопытки
			Иначе
				Результат = ОбщийМодульОбщегоНазначения.ВычислитьВБезопасномРежиме(Алгоритм + "(" + АргументыВызова + ")", Параметры);
			КонецЕсли;	
		КонецЕсли;
	Иначе	
#Область include_IntegrationBlockly_base_Blocks_C1CalculateNoSafeMode	
#КонецОбласти	
	КонецЕсли;	
...
Функция block_c1_calculate_calc_value(block_type, node, path, context, block_context) 
	...
	Попытка
		Если Не block_context.Свойство("PARAM0") Тогда
			...
			АдресРезультата = c1_calculate_calc_value(Алгоритм); 
		...
		Иначе
			...
			КонецЦикла;    
			АдресРезультата = c1_calculate_calc_value(block_context.NAME, АргументыВызова, ПараметрыАлгоритма, ПараметрыАлгоритмаМассив);
			НомерПараметра = 0;
			Пока block_context.Свойство("PARAM" + Строка(НомерПараметра)) Цикл
				Если ТипЗнч(get_prop(block_context, "PARAM" + Строка(НомерПараметра))) = Тип("Соответствие") Тогда
	...
	Исключение  
		...
		ВызватьИсключение NewExtExceptionСтрока(ИнфОбОшибкеБл,,,,add_block_to_dump(block_context));;	
	КонецПопытки;
	end = ДатаВМиллисекундах();
