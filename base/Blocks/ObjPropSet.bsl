...
Процедура block_obj_prop_set_inobj1с(obj, obj_path, value)  
	delimiter = ".";
	_path = Saby_СтрРазделить82(obj_path, delimiter, Ложь);
	_obj = obj;
...
Функция block_obj_prop_set_calc_value(block_type, node, path, context, block_context)
	...
	Если ТипЗнч(obj_path) <> Тип("Строка") ИЛИ obj_path = "" Тогда
		ВызватьИсключение "Неверный тип параметра ""свойство"". Необходимо передавать только непустую строку."; 
	КонецЕсли;
	Попытка
