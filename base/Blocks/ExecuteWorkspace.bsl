Функция block_execute_workspace_calc_value(block_type, node, path, context, block_context)
	params_count = Число(workspace_find_mutation_by_name(node, "param_count", 0));
	...
	Если get_prop(block_context, "_child") = Неопределено Тогда
	    ...
	    Для i = 1 по params_count Цикл
	        key = block_context["PARAM_NAME_" + Строка(i)];
	        value = block_context["PARAM_VALUE_" + Строка(i)];
	        block_context["_child"]["variable_scopes"][0].Вставить(key, value);
		КонецЦикла;	
	КонецЕсли;
	Попытка
		save_last_context();
