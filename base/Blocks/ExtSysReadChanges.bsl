...
Процедура ПроверкаОбъектаПоФильтру(СсылкаНаОбъект, Фильтр, МетаданныеСсылки, СтруктураОтвета)
	...
	Отступ3 = 3;
	Сравнение = "НеРавно";
	Если Прав(НазваниеФильтра, 5) = "_From" Тогда 
		...
		Сравнение = "Меньше";
	КонецЕсли; 
	Если Прав(НазваниеФильтра, 3) = "_To" Тогда 
...
Функция ПолучитьИзменения(ИмяПланаОбмена, ИдентификаторУзлаОбмена, ТаблицаФильтров, ОбъектыВыгрузки, context)
...
Процедура ЗаписатьИзмененияУзлаВExtsyncdoc(context, ИмяПланаОбмена, connection_uuid)
	...
	МассивПакетыОбъектов = ПолучитьИзменения(ИмяПланаОбмена, connection_uuid, ТаблицаФильтров, ОбъектыВыгрузки, context);	
	Для Каждого ПакетОбъектов Из МассивПакетыОбъектов Цикл
		objects = block_extsyncdoc_write_api3_link(context, direction, ПакетОбъектов, Истина);
		ПолученныйUuid = ТранспортИнтеграции.local_helper_extsyncdoc_write(
			context.params, connection_uuid, ПараметрыВызова, objects);
	КонецЦикла;
КонецПроцедуры
