...
Функция block_saby_read_document_get_value(block_context, context, ПараметрыВызова, multithread_mode)
	...
	Если QueryId = Неопределено Тогда  		
		Попытка
			Результат = Транспорт.local_helper_read_document(context.params, ПараметрыВызова, multithread_mode, 120);
		Исключение
	...
	Иначе 
		...
		Результат = Транспорт.local_helper_read_document_process_responce(responce);
