...
Функция block_extsyncdoc_run_calc_value(block_type, node, path, context, block_context)
    ...
    Транспорт.local_helper_pause(1);
	return result
КонецФункции
	...
	Если result["status"] = "complete" Тогда
		Если ТипЗнч(result["data"]) = Тип("Массив") Тогда 
			Для каждого _data из result["data"] Цикл
				Транспорт.local_helper_extsyncdoc_write_predefined(context["params"], context["operation"]["connection_uuid"], ext_sync_obj["Type"], _data["ClientType"], _data["Objects"]);	
			КонецЦикла;	
		Иначе	
			Транспорт.local_helper_extsyncdoc_write_predefined(context["params"], context["operation"]["connection_uuid"], ext_sync_obj["Type"], result["data"]["ClientType"], result["data"]["Objects"]);  
		КонецЕсли;  
...
Функция block_extsyncdoc_run_command_getobject(context, block_context, ext_sync_obj, objects)
	...
	Если data_is = Неопределено Тогда
		...
		data_is.Вставить("Регламент", get_prop(get_prop(ext_sync_obj,"Data"),"Регламент"));
		data_is.Вставить("ПроизвольноеНазваниеРегламента", get_prop(get_prop(ext_sync_obj,"Data"),"ПроизвольноеНазваниеРегламента"));
		data_is.Вставить("ini_name", СтрЗаменить(get_prop(get_prop(ext_sync_obj,"Data"),"ini_name"),"СинхВыгрузка_",""));
		data_is.Вставить("_print_forms", get_prop(get_prop(ext_sync_obj,"Data"),"_print_forms")); //TODO убрать костыли
	КонецЕсли;
	obj.Вставить("object", data_is);
...
Функция block_extsyncdoc_run_command_find(context, block_context, ext_sync_obj, objects, item_keys)
	...
	Если get_prop(obj, "ИдИС") <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(result) Тогда
			...
			Транспорт.local_helper_mapping_obj_find_and_update(
			context.params,
			_filter,
			_data
			);
...
Функция load_calc_ini(ini_name, obj, endpoint)
	...
	add_new_context();
	Попытка
		ПараметрыВызова = Новый Структура("algorithm, object, endpoint", ini_name, obj, endpoint);
...
Процедура block_extsyncdoc_run_process_command_result_read(context, block_context, ext_sync_obj, result, objects, action = Неопределено)
	...
	ИначеЕсли result.status = "error" Тогда
		...
		Если get_prop(ОшибкаСтруктура,"type","") = "UserCancel" Тогда
			_data.Вставить("StatusId", "Игнорирован");
			data_is = block_obj_get_path_value(_data,"Data.data_is",Неопределено);
			Если data_is = Неопределено Тогда
				data_is = Новый Структура;
			КонецЕсли;
...
Процедура block_extsyncdoc_run_process_command_result_read_array(context, block_context, ext_sync_obj, result, objects, action)
	...
	Для каждого _result из result Цикл
		...
		КонецЕсли;
		ОбъектСинхронизации = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(ext_sync_obj));
...
Функция block_extsyncdoc_run_process_command_result_update(context, block_context, command, result, objects, action)
	...
	Если get_prop(result["data"], "ИдИС") <> Неопределено Тогда
		...
		Транспорт.local_helper_mapping_obj_find_and_update(context.params, _filter, _data);
	...
	ИначеЕсли result["status"] = "complete" И  Не ЗначениеЗаполнено(result["data"]) Тогда
		_filter = Новый Структура("ConnectionId, Type, Id, IdType",
			connection_uuid,
			command["Data"]["data"]["ИмяСБИС"],
			command["Data"]["data"]["ИдСБИС"],
			...
			Для каждого ik из Keys Цикл
				...
				Пока Истина Цикл
					Если get_prop(ik, "Value"+ИндексКлюча) <> Неопределено Тогда
						...
						КонецЕсли;
						Если НомерКлюча = 1 Тогда
							_data.Вставить("ClientParam_"+НомерКлюча+"_"+ИндексКлюча, ЗначениеКлюча);
						ИначеЕсли НомерКлюча = Keys.Количество() Тогда
							_data.Вставить("ClientParam_ObjectName", ЗначениеКлюча);
						Иначе
							_data.Вставить("ClientParam_"+НомерКлюча, ЗначениеКлюча);
						КонецЕсли;
...
Функция block_extsyncdoc_run_extsyncdoc_prepare_is(context, block_context)
	...
	Пока Истина Цикл
		...
		result = Неопределено;
		Попытка
			result = Транспорт.local_helper_extsyncdoc_prepare(context.params, extsyncdoc_uuid, 1);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
		...
		Если count_actions > 0 Тогда
			...
			Если objects.Количество() > 0 Тогда
				res = Транспорт.local_helper_extsyncdoc_write(
				context.params,
				connection_uuid,
...
Функция block_extsyncdoc_run_extsyncdoc_prepare_saby(context, block_context)
	extsyncdoc_uuid = "";
	context.operation.Свойство("operation_uuid", extsyncdoc_uuid);
	result = Транспорт.local_helper_extsyncdoc_prepare_saby(context.params, extsyncdoc_uuid);
...
Функция block_extsyncdoc_run_extsyncdoc_execute_saby(context, block_context)
	extsyncdoc_uuid = "";
	context.operation.Свойство("operation_uuid", extsyncdoc_uuid);
	Возврат Транспорт.local_helper_extsyncdoc_execute(context.params, extsyncdoc_uuid, 1);
КонецФункции
...
Функция block_extsyncdoc_run_extsyncdoc_execute_saby_lrs(context, block_context)
	extsyncdoc_uuid = "";
	context.operation.Свойство("operation_uuid", extsyncdoc_uuid);
	Транспорт.local_helper_extsyncdoc_execute_lrs(context.params, extsyncdoc_uuid, 1);
	Пока Истина Цикл
		result = block_extsyncdoc_run_extsyncdoc_read_saby(context, block_context);
		ДопПарамерыПрогресса = ПрогрессВыполнения(result);
		ТекстСтатуса = "Загружено " + result["CountConfirmed"]+"/"+result["CountObjects"]+", ошибок "+result["CountErrors"];
		Если result["CountObjects"] > 0 и result["CountObjects"] = (result["CountErrors"] + result["CountConfirmed"] + result["CountProcessed"]) Тогда
			СообщитьПрогрессОперации(, ТекстСтатуса, ДопПарамерыПрогресса);
			Транспорт.local_helper_pause(1); // Если не поставить паузу, то сообщение может не успеть долететь и обновить строку состояния выгрузки.
			Прервать;
		Иначе			
...
Функция block_extsyncdoc_run_extsyncdoc_execute_is(context, block_context, break_extsyncobj_uuid=Неопределено)
	...
	Пока Истина Цикл
		...
		Попытка
			result = Транспорт.local_helper_extsyncobj_get_obj_for_execute(context.params, extsyncdoc_uuid, extra_fields, limit);
		Исключение 
			ИнфОбОшибке = ИнформацияОбОшибке();
			ВызватьИсключение(NewExtExceptionСтрока(ИнфОбОшибке));
		КонецПопытки;
		...
		Если Не ЗначениеЗаполнено(result) Тогда
			Транспорт.local_helper_extsyncdoc_execute(context.params, extsyncdoc_uuid, 2);
			Транспорт.local_helper_pause(1);
			Возврат Истина;
		КонецЕсли;
		...
		СообщитьПрогрессОперации(, ТекстСтатуса, ДопПарамерыПрогресса);
	КонецЦикла;
	Транспорт.local_helper_pause(1); //Для прогрузки состояния на форме длительных операций
КонецФункции
...
Функция block_extsyncdoc_run_extsyncdoc_read_saby(context, block_context)
	extsyncdoc_uuid = "";
	context.operation.Свойство("operation_uuid", extsyncdoc_uuid);
	Возврат Транспорт.local_helper_extsyncdoc_read(context.params, extsyncdoc_uuid);
КонецФункции
