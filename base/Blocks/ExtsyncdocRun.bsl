...
Функция block_extsyncdoc_run_command_processpredefineobject(context, block_context, ext_sync_obj)
	...
	obj.Вставить("ИмяСБИС", СтрЗаменить(ext_sync_obj.Получить(_Data).Получить("ini_name"), "СинхВыгрузка_", "")); 
	...
	Если result["status"] = "complete" Тогда
		Если ТипЗнч(result[_DataSm]) = Тип("Массив") Тогда 
			Для каждого _data из result[_DataSm] Цикл
		...
		Иначе	
			ТранспортИнтеграции.local_helper_extsyncdoc_write_predefined(
				...
				context["operation"]["connection_uuid"],
				ext_sync_obj[_Type],
				result[_DataSm]["ClientType"],
				result[_DataSm]["Objects"]);  
		КонецЕсли;  
	Иначе
...
Функция block_extsyncdoc_run_command_getobject(context, block_context, ext_sync_obj, objects)
	...
	Если data_is = Неопределено Тогда
		...
		data_is.Вставить("ini_name", СтрЗаменить(get_prop(get_prop(ext_sync_obj, _Data), "ini_name"), "СинхВыгрузка_", ""));
...
Процедура ЗаполнитьClientParamОбъекта(_data, Keys)
	...
	КонецЦикла;
	Попытка
		result = ПолучитьСсылкуПоИдИС(get_prop(_data,"ClientType"), get_prop(_data,"ClientId")); 
		_data.Вставить("ClientName", Строка(result));
	Исключение
		result = Неопределено;	
	КонецПопытки;	
КонецПроцедуры	
