	...
	Если calc_find Тогда
		Для Каждого item_key из item_keys Цикл
			...
			Если result["status"] = "complete" и ЗначениеЗаполнено(result["data"]) Тогда
				...
				_data = Новый Структура("SbisType, SbisId", ext_sync_obj["Data"]["data"]["ИмяСБИС"], ext_sync_obj["Data"]["data"]["ИдСБИС"]);
				_data.Вставить("Integration", Integration);
				_data.Вставить("ClientId", result["data"]["ИдИС"]);
				_data.Вставить("ClientType", result["data"]["ИмяИС"]);
				_data.Вставить("Status", ?(ЗначениеЗаполнено(result["data"]["ИдИС"]), 1, 4));
				_data.Вставить("StatusMsg", "Сопоставлено");
			...
			ИначеЕсли result["status"] = "error"  и get_prop(result["data"], "Type") <> "NotFound" Тогда
				...
				_data.Вставить("StatusMsg", get_prop(result["data"], "message"));	
...
Процедура block_extsyncdoc_run_process_command_result_read(context, block_context,
	...
	ИначеЕсли result.status = "error" Тогда
		...
		Если ЭтоПользовательскоеИсключение(type) Тогда
			...
			data_is = block_obj_get_path_value(_data, "Data.data_is", Новый Структура);
...
Функция ПроверкаЗначенийПоФильтру(data_is, КлючФильтра, ЗначениеФильтра)
	ЭтоТипСтрокаЧисло = Новый Массив;
	ЭтоТипСтрокаЧисло.Добавить(Тип("Строка"));
	ЭтоТипСтрокаЧисло.Добавить(Тип("Число"));
	Если ЭтоТипСтрокаЧисло.Найти(ТипЗнч(ЗначениеФильтра)) <> Неопределено Тогда 
