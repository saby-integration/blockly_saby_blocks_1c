...
Функция block_extsyncdoc_run_calc_value(block_type, node, path, context, block_context)
	Если context.is_deferred <> Неопределено Тогда
		block_extsyncdoc_run_calc_value_deferred(block_type, node, path, context, block_context);	
    КонецЕсли;
    ...
    report_write_unregistered_objects(extsyncdoc_params());
	...
	ОтображатьПрогрессВыполнения = get_prop(block_context, "ОтображатьПрогрессВыполнения", 
		get_prop(context, "current_workspace") <> "Документы_execute");	 
...
Процедура block_extsyncdoc_run_calc_value_deferred(block_type, node, path, context, block_context)
	Попытка
		deferred_index = helper_find_in_array(context.deferred, context.is_deferred, "__deferred");	
	Исключение   
		ВызватьИсключение NewExtExceptionСтрока(, "Deferred not found",,, "Exception");
	КонецПопытки;
	deferred = context.deferred[deferred_index];
	deferred_block_context = deferred["block_context"];
	objects = Новый Массив;
	Если deferred_block_context["command_name"] = "update" Тогда
		block_extsyncdoc_run_command_update(context, deferred_block_context, deferred_block_context["ext_sync_obj"], objects);
	ИначеЕсли deferred_block_context["command_name"] = "read" Тогда
		block_extsyncdoc_run_command_getobject(context, deferred_block_context, deferred_block_context["ext_sync_obj"], objects);
	Иначе
		ВызватьИсключение "Неизвестная отложенная команда";
	КонецЕсли;	
...
Функция block_extsyncdoc_run_command_syncdocfill(context, block_context, ext_sync_obj, objects)
	...
	ФильтрРодитель	= Новый Соответствие;
	ФильтрРодитель.Вставить("Родитель", ext_sync_obj["filter"]["parent"]);	
	EndpointArgs = Новый Структура("Filter", ФильтрРодитель);
	Страниц	= 0;
	...
	Пока ЕстьЕще = Истина Цикл
		...
		result = block_extsyncdoc_run_add_command_calc_ini(context, block_context, "list", ext_sync_obj, obj, , EndpointArgs);
		
		_Pagination = get_prop(get_prop(result, "Result"),"Pagination",Новый Структура);
		ЕстьЕще = get_prop(_Pagination,"HasMore",Ложь);	
		Страниц = Страниц + 1;
		
		Если get_prop(result, "Status", "") = "complete" Тогда
			ЗаписиСтрок = get_prop(get_prop(result, "Result"), "Rows", Новый Массив);
			Для Каждого СтрокаЗаписи из ЗаписиСтрок Цикл
				НоваяЗапись = Новый Структура;
				НоваяЗапись.Вставить("ИдИС", get_prop(СтрокаЗаписи, "ИдИС"));
				НоваяЗапись.Вставить("ИмяИС", get_prop(СтрокаЗаписи, "ИмяИС"));
				НоваяЗапись.Вставить("ИмяСБИС", get_prop(СтрокаЗаписи, "ИмяСБИС"));
				
				obj = Новый Соответствие();
				obj.Вставить("object", НоваяЗапись);
...
Функция block_extsyncdoc_run_command_getobject(context, block_context, ext_sync_obj, objects)
	...
	Если data_is = Неопределено Тогда
		...
		Иначе
			Type = "Type";
		КонецЕсли;	
		type_list = Saby_СтрРазделить82(get_prop(ext_sync_obj, Type), ".");
		data_is	= Новый Соответствие;
...
Функция block_extsyncdoc_run_command_find(context, block_context, ext_sync_obj, objects, item_keys)
	...
	Если get_prop(obj, "ИдИС") <> Неопределено Тогда
		Попытка
			...
			result.Вставить("Result", ПолучитьСсылкуПоИдИС(obj["ИмяИС"], obj["ИдИС"]));
		Исключение
			result = Неопределено;	
		КонецПопытки;	
		Если ЗначениеЗаполнено(result) Тогда
			...
			ТранспортИнтеграции.local_helper_mapping_obj_write_by_sbis_id(context.params, _data);
...
// EndpointArgs - Структура - EndpointArgs
//
// Возвращаемое значение:
//  Структура - Результат выполения функции
//
...
Функция block_extsyncdoc_run_add_command_calc_ini(context, block_context,
	command, ext_sync_obj, obj, endpoint = Неопределено, EndpointArgs = Неопределено)
	Если get_prop(ext_sync_obj, "ClientType") <> Неопределено
		И command <> "update" Тогда
		Type = "ClientType";
	Иначе
		Type = "Type";
	КонецЕсли;
	type_list = Saby_СтрРазделить82(ext_sync_obj[Type], ".");
	...
	Если Не ЗначениеЗаполнено(ini_name) Тогда
		...
		Если ЗначениеЗаполнено(sbis_type) Тогда
			ini_name = "Blockly_" + is_type + sbis_type + "_" + command;
			Попытка
				Load_ini(ini_name);
			Исключение
				ini_name = "Blockly_" + is_type + "_" + command;
			КонецПопытки;
		Иначе
			ini_name = "Blockly_" + is_type + "_" + command;
		КонецЕсли;	
	...
	Возврат load_calc_ini(ini_name, obj, endpoint, block_context, EndpointArgs);
КонецФункции
...
//
//DynamicDirective
Функция load_calc_ini(ini_name, obj, endpoint, block_context = Неопределено, EndpointArgs = Неопределено)
	Load_ini(ini_name);
	Data = get_prop(context.report, "data");
	add_new_context();
	Если get_prop(block_context, "deferred") <> Неопределено И block_context.deferred.Количество() Тогда
		context.deferred = block_context.deferred;
		context.block_context = block_context.block_context;
		context.variable_scopes = block_context.variable_scopes;
	КонецЕсли;	
	Попытка
		OperationParams = get_prop(Data, "EndpointArgs", Новый Массив);
		// BSLLS:FunctionOutParameter-off
		Если EndpointArgs = Неопределено Тогда
			EndpointArgs = Новый Структура;
		КонецЕсли;	
		EndpointArgs.Вставить("object", obj);
		EndpointArgs.Вставить("OperationParams", OperationParams);
		...
		Если context.deferred.Количество() > 0 Тогда	
			block_context.Вставить("deferred", Saby_КопияМассива(context.deferred));
			block_context.Вставить("block_context", Saby_КопияСтруктуры(context.block_context)); 
			block_context.Вставить("variable_scopes", Saby_КопияМассива(context.variable_scopes,, context.variable_scope_current));
			ИнтеграцияДополнитьМассив(contexts[contexts.Количество() - 1].commands, context.commands);
		КонецЕсли;	
		report_update();
		delete_last_context();
		Возврат Результат;
...
Процедура block_extsyncdoc_run_process_command_result_read(context, block_context,
	...
	Если result["Status"] = "run" Тогда 
		ОбработатьОтложеннуюОперациюКоманды(context, block_context, ext_sync_obj, "read");
	КонецЕсли;
...
Процедура block_extsyncdoc_run_process_command_result_read__object(result, ext_sync_obj, objects, action = Неопределено)
	Попытка
		ПроверитьНаличиеОбязательныхПараметров(result, "ИдИС,ИмяИС", get_prop(ext_sync_obj, "Type", "")+" "+get_prop(ext_sync_obj,"Title",ext_sync_obj["ClientId"]));
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
...
Процедура ОбработатьОтложеннуюОперациюКоманды(context, block_context, command, command_name)
	uuid_deferred = get_prop(block_context, "is_deferred");
	Если Не ЗначениеЗаполнено(uuid_deferred) Тогда
		uuid_deferred = Строка(Новый УникальныйИдентификатор);	
		block_context.Вставить("is_deferred", uuid_deferred);
		block_context.Вставить("ext_sync_obj", command);
		block_context.Вставить("command_name", command_name);
	КонецЕсли;
	
	dump = Новый Структура; 
	dump.Вставить("uuid", uuid_deferred); 
	dump.Вставить("multi_thread_loop_block_context", Saby_КопияСтруктуры(block_context));			
	dump.Вставить("multi_thread_loop_variable_scopes", Saby_КопияМассива(context.variable_scopes, context.variable_scope_current + 1));
...
Функция block_extsyncdoc_run_process_command_result_update(context, block_context, command, result, objects, action)
	Если result["Status"] = "run" Тогда 
		ОбработатьОтложеннуюОперациюКоманды(context, block_context, command, "update");
	КонецЕсли;	
...
Функция СравнениеПоИмяИсИдИС(ЗначениеИС, МассивЗначенийФильтра, ЭтоТипСтруктураСоответствие) 
	...
	Для Каждого ЗаписьЗначенияФильтра Из МассивЗначенийФильтра Цикл
		...
		Если ИдИс = ИдИсФильтра Или Лев(ИдИс, 36) = ИдИсФильтра Тогда
			Возврат Ложь;
		КонецЕсли;	
		СсылкаЗначенияИС = ПолучитьСсылкуПоИдИС(ИмяИс, ИдИс);
		СсылкаФильтра = ПолучитьСсылкуПоИдИС(ИмяИсФильтра, ИдИсФильтра);
		Если ПроверкаПринадлежностиЭлементу(СсылкаЗначенияИС, СсылкаФильтра) Тогда
			Возврат Ложь;
		Иначе
			Игнорирован = Истина;	
		КонецЕсли;
...
Процедура ПроверитьОбъектыПоФильтрам(context, objects)
	...
	Для Каждого ext_sync_obj Из objects Цикл 
		...
		Для Каждого Запись_Filter Из Filter Цикл
			Если Не ЗначениеЗаполнено(Запись_Filter["Значение"]) Тогда  
				Продолжить;
			КонецЕсли;
...
Процедура block_extsyncdoc_run_commands_write_obj(context, objects)
	Если objects.Количество() > 0 Тогда 
		data_to_write = get_prop(context.report, "data_to_write", Новый Структура);
		operation = get_prop(context, "operation"); 
		connection_uuid = get_prop(operation, "connection_uuid");
...
Процедура block_extsyncdoc_run_commands_execute_command(context, block_context, command_, objects)
...
Процедура block_extsyncdoc_run_commands(ПараметрыВыполнения, РезультатВыполнения = Неопределено) Экспорт
	...
	Для Каждого command_ Из actions Цикл
		Попытка
			block_extsyncdoc_run_commands_execute_command(context, block_context, command_, objects);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			Если ОшибкаСтруктура.type = "DeferredOperation" Тогда
				context_add_deferred(context, ОшибкаСтруктура.dump);
				block_context.Вставить("is_deferred", Неопределено);
				block_context.Вставить("ext_sync_obj", Неопределено); 
				block_context.Вставить("deferred", Новый Массив); 
				block_context.Вставить("block_context", Неопределено);
				Продолжить;
			КонецЕсли;
	...
	block_extsyncdoc_run_commands_write_obj(context, objects);
КонецПроцедуры
...
Процедура check_run_commands_background_job(РезультатыПотоков, context)
	...
	Для Каждого Запись Из РезультатыПотоков Цикл 
		...
		Для Каждого action Из МассивОбъектов Цикл 
			...
			_object.Вставить("StatusId", StatusId);
			_object.Вставить("StatusMsg", StatusMsg);
			_object.Вставить("Data", Новый Структура("error", get_prop(Запись.Значение, "error", Неопределено)));
			obj_Uuid = get_prop(obj, "Uuid"); 
			Если ЗначениеЗаполнено(obj_Uuid) Тогда 
				_object.Вставить("Uuid", obj_Uuid);
			Иначе
				_object.Вставить("ClientType", get_prop(obj, "ClientType"));
				_object.Вставить("ClientId", get_prop(obj, "ClientId"));
				_object.Вставить("SbisType", get_prop(obj, "SbisType"));
			КонецЕсли;	
			objects.Добавить(_object);			
		КонецЦикла;	
	КонецЦикла;
