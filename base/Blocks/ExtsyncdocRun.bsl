...
Процедура ОжиданиеЗавершенияПотоков(МассивПотоков) 
	ФоновыеЗадания.ОжидатьЗавершения(МассивПотоков, 1800);
КонецПроцедуры	
...
Процедура ЗапуститьМногопоточноеВыполнение(МассивОбъектов, ИмяФункции, ПараметрыВыполнения, ИмяПараметраОбъектов, context, НаименованиеФоновогоЗадания)
	...
	Для Каждого Объект Из МассивОбъектов Цикл 
		...
		Если ОбъектыПотока.Количество() >= КоличествоОбъектовВПотоке Тогда
		...
		Если МассивПотоков.Количество() >= КоличествоПотоков Тогда
			ОжиданиеЗавершенияПотоков(МассивПотоков);
			МассивПотоков = Новый Массив;
		КонецЕсли;	
	КонецЦикла;
...
Процедура block_extsyncdoc_run_commands_background_job(context, block_context, actions)
	...
	ЗапуститьМногопоточноеВыполнение(actions, "block_extsyncdoc_run_commands", ПараметрыВыполнения, "actions", context, ИмяФоновогоЗадания); 
КонецПроцедуры
...
Функция block_extsyncdoc_run_commands_update_background_job(context, block_context, actions, break_extsyncobj_uuid)
	...
	ЗапуститьМногопоточноеВыполнение(actions, "block_extsyncdoc_run_commands_update", ПараметрыВыполнения, "actions", context, ИмяФоновогоЗадания);
	Возврат Неопределено;
КонецФункции	
