...
Функция block_indicator_from_accounting_entry_side_calc_value(block_type, node, path, context, block_context)
	...
	Если ТипЗнч(ПодходящийПоказательПриемник) = Тип("Массив") Тогда
		ПодходящийПоказательПриемник = ПодходящийПоказательПриемник[0];
	КонецЕсли;
...
//
// Параметры:
// indicator_code - Строка - Код счёта
//
//DynamicDirective
...
Функция ИнициализироватьТаблицупоказателей()
	...
	indicator_list.Колонки.Добавить("СпособХранения", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(1000)));
