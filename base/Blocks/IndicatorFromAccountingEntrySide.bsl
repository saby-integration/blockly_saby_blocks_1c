	...
	Для i = block_context.ИндексПодходящегоПоказателя По ПодходящиеПоказателиИсточник.Количество()-1 Цикл
		...
		Для Каждого ИдентифПарам Из Стр["Параметры"] Цикл
			...
			Если ЗначениеЗаполнено(ИдентифПарам.Значение) Тогда
				handler = Functions[ИдентифПарам.Значение[0]];
				...
				Если block_context._КонтекстРасчетаПринадлежности = Неопределено Тогда
					block_context._КонтекстРасчетаПринадлежности = Новый Структура("СторонаПроводки",  block_context.accounting_entry_side);
					block_set_variable(context, "СторонаПроводки", block_context.accounting_entry_side);
					Попытка
						Для счПарам = 1 По ИдентифПарам.Значение.Количество()-1 Цикл
							...
							block_set_variable(context, args_values.args[счПарам], ИдентифПарам.Значение[счПарам]);
						КонецЦикла;
					Исключение
					...
					КонецЕсли;
				КонецЕсли;
				Результат = block_execute(handler.type,handler.node, path, context, block_context._КонтекстРасчетаПринадлежности);
		...
		Если block_context.blacklist_detalization["Список"].Найти(Детал.Ключ) = Неопределено Тогда
			Если ЗначениеЗаполнено(Детал.Значение) и ЗначениеЗаполнено(Детал.Значение["СпособПолучения"]) Тогда
				...
				Если ЗначениеЗаполнено(ФункцияЭкспорт) Тогда
					handler = Functions[ФункцияЭкспорт];
					args_values = procedures_callreturn_get_params(handler.node);
					Если block_context._КонтекстРасчетаЭкспортаДетализации = Неопределено Тогда
						block_context._КонтекстРасчетаЭкспортаДетализации = Новый Структура("СторонаПроводки",  block_context.accounting_entry_side);
						block_set_variable(context, "СторонаПроводки", block_context.accounting_entry_side);
						Попытка
							Для счПарам = 1 По СпособПолученияЭкспорт.Количество()-1 Цикл
								...
								block_set_variable(context, args_values.args[счПарам], СпособПолученияЭкспорт[счПарам]);
							КонецЦикла;
						Исключение
							ИнфОбОшибке = ИнформацияОбОшибке();
						...
						КонецЕсли;
					КонецЕсли;
					РезультатРасчета = block_execute(handler.type,handler.node, path, context, block_context._КонтекстРасчетаЭкспортаДетализации);
				...
				Если ЗначениеЗаполнено(РезультатРасчета) Тогда
					...
					Если ЗначениеЗаполнено(ФункцияИмпорт) Тогда
						handler = Functions[ФункцияИмпорт]; 
						args_values = procedures_callreturn_get_params(handler.node);
						Если block_context._КонтекстРасчетаИмпортаДетализации = Неопределено Тогда
							block_context._КонтекстРасчетаИмпортаДетализации = Новый Структура("СторонаПроводки, Аналитика",  block_context.accounting_entry_side, РезультатРасчета);
							block_set_variable(context, "СторонаПроводки", accounting_entry_side_result);
							block_set_variable(context, "Аналитика", РезультатРасчета);
							Попытка
								Для счПарам = 1 По СпособПолученияИмпорт.Количество()-1 Цикл
									...
									block_set_variable(context, args_values.args[счПарам+1], СпособПолученияИмпорт[счПарам]);
								КонецЦикла;
							Исключение
								ИнфОбОшибке = ИнформацияОбОшибке();
							...
							КонецЕсли;
						КонецЕсли;
						accounting_entry_side_result = block_execute(handler.type,handler.node, path, context, block_context._КонтекстРасчетаИмпортаДетализации);
