Функция block_extsyncdoc_create_calc_value(block_type, node, path, context, block_context)
	...
	Исключение
		ДанныеОшибки = ExtException(
			Новый Структура("message, detail", "Не указан обязательный параметр", block_type + " Data"));
		ОшибкаСтрокой = ЗначениеВСтрокуВнутр(ДанныеОшибки);
		ВызватьИсключение ОшибкаСтрокой;
	КонецПопытки;
	...
	Попытка
		...
		ПолученныйUuid = local_helper_extsyncdoc_write(
			context.params,
			connection_uuid,
			ПараметрыВызова,
			Неопределено);
