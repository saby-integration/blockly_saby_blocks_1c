...
Функция block_c1_ref_from_api3_calc_value(block_type, node, path, context, block_context)
	...
	КонецЕсли;
	Если Найти(get_prop(block_context.API3, "ИмяИС", ""), ".") > 0 Тогда
		ИмяИС = get_prop(block_context.API3, "ИмяИС", "");
	Иначе
		ИмяИС = get_prop(block_context.API3, "ТипИС", "") + "." + get_prop(block_context.API3, "ИмяИС", "");
	КонецЕсли;
	ИдИС = get_prop(block_context.API3, "ИдИС", "");
	Subtitle = Строка(ИмяИС) + " " + Строка(ИдИС); 
	Попытка
		СсылкаНаобъект = ПолучитьСсылкуПоИдИС(ИмяИС, get_prop(block_context.API3, "ИдИС", ""));
	Исключение
