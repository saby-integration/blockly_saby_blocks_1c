...
Процедура ИнициализироватьВыборку(node, path, context, block_context, Pagination, local_context)
	Если Не local_context.Свойство("_Выборка") Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПЕРВЫЕ 0", "ПЕРВЫЕ " + Формат(block_context["LIMIT"] + 1, "ЧГ=0"));
...
Процедура c1_call_select2_execute(block_type, node, path, context, block_context, Pagination, local_context)
	Пока Истина Цикл
		...
		EXCLUDED_RECORDS = get_prop(block_context, "EXCLUDED_RECORDS", Новый Массив);
		excluded = index_excluded(EXCLUDED_RECORDS);
		
		Попытка
			ДочернийУзел = get_statement_node(node);
			ОбсчитатьЗаписьВыборки(ДочернийУзел, path, context, block_context, excluded);			
		Исключение   
			ИнфОбОшибке = ИнформацияОбОшибке(); 
			ПрерватьЦикл = Ложь;
