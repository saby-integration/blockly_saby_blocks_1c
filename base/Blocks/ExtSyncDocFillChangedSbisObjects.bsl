...
Функция block_extsyncdoc_extsyncdoc_fillchangedsbisobjects_calc_value(block_type, node, path, context, block_context) Экспорт  
	connection_uuid = "";
	ExtSyncDoc = Новый Структура("Uuid", get_prop(context.report, "uuid"));
	context.operation.Свойство("connection_uuid", connection_uuid);
	Если context.report.new = Истина Тогда
		context.report.new = Ложь;
		context.report.Вставить("direction", 0);
		report_write_unregistered_objects(extsyncdoc_params());	
	КонецЕсли;	
	extsyncdoc_uuid = ТранспортИнтеграции.local_helper_extsyncdoc_fillchangedsbisobjects_lrs(context.params, connection_uuid, ExtSyncDoc);
