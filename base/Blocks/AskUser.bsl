...
// BSLLS:UnusedLocalVariable-on
		_ДанныеИсключения = Новый Структура("uuid, context, block_context", Строка(__deferred), context, block_context);
	...
	Иначе
		...
		Если command_result.status = "complete" Тогда   
			block_context.Вставить("result", command_result.result);
