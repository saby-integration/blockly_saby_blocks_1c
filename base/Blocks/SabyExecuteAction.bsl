...
Функция block_saby_execute_action_calc_value(block_type, node, path, context, block_context)
	...
	Если get_prop(block_context, "_prepare_result") = Неопределено Тогда
		...
		Если attachments.Количество() > 0 Тогда
			...
			Попытка
				...
				Транспорт.local_helper_write_attachment(context.params, ДокДляВложений);
			Исключение
	...
	Если ВидТранспорта = "ExtSdk2" Или ВидТранспорта = "SabyPluginConnector" Тогда
		Возврат Транспорт.ExecuteActionEx(block_context, context);
	КонецЕсли;
...
Функция block_saby_execute_action_magic_button(context, doc, doc_attach, doc_performer)
	...
	Для Сч=0 По 2 Цикл
		Попытка
			result = Транспорт.local_helper_request_signing(context, params);
			Возврат result;
		Исключение   
			ИнфОбОшибке = ИнформацияОбОшибке();
			Транспорт.local_helper_pause(5);
		КонецПопытки;
	КонецЦикла;	
		...
		Для каждого action из get_prop(stage, "Действие") Цикл
			...
			Если get_prop(action, "ТребуетПодписания") = "Да" И Сертификат.Количество() > 0 Тогда
				Попытка
					...
					КонецЦикла;	                                                                            
					operation_uuid = Транспорт.local_helper_init_remote_signing(context.params, sign_array);
				Исключение
					ИнфОбОшибке = ИнформацияОбОшибке();
					Возврат Новый Массив;	
				КонецПопытки;
				Попытка
					Возврат Транспорт.local_helper_get_remote_signature(context.params, operation_uuid);	
				Исключение
					ИнфОбОшибке = ИнформацияОбОшибке();
					Возврат Новый Массив;
				КонецПопытки;	
...
Функция block_saby_execute_action_continue(block_context, context) Экспорт 
	...
	Если get_prop(block_context, "_prepare_result") = Неопределено Тогда
		...
		Попытка
			block_context.Вставить("_prepare_result", Транспорт.local_helper_prepare_action(context.params, doc1));
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();			
	...
	Попытка
		...
		Возврат Транспорт.local_helper_execute_action(context.params, block_context.execute_action_param);
	Исключение
