		...
		Если attachments_count > 0 Тогда
			...
			Исключение   
				...
				Если object <> Неопределено Тогда
					report_add_objects(object); 
				КонецЕсли;	
				block_set_variable(context, "_last_error", ОшибкаСтруктура);
				Возврат Ложь;
			КонецПопытки;
			Data = Новый Структура;
...
Функция block_saby_execute_action_magic_button(context, doc, doc_attach, doc_performer)
	...
	Для каждого Сотрудника Из doc_performer Цикл
		Если doc["КаналИнформации"] = "0" Тогда
			Контакт = Сотрудника["ЭлПочта"];
		ИначеЕсли doc["КаналИнформации"] = "1" Тогда
			Контакт = Сотрудника["МобильныйТелефон"];
		Иначе
			Контакт = Неопределено;
		КонецЕсли;
		Если Сотрудника["Роль"] = "Руководитель" И Контакт <> Неопределено Тогда	  
			мРуководители.Добавить(Новый Структура("PersonnelNumber, Contact", Сотрудника["ТабельныйНомер"], Контакт)); 	
		Иначе
			Если Сотрудника["Роль"] = "Сотрудник" И Контакт <> Неопределено Тогда	 
				мСотрудники.Добавить(Новый Структура("PersonnelNumber, Contact", Сотрудника["ТабельныйНомер"], Контакт)); 
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
