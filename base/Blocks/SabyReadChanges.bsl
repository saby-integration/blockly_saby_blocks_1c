...
Функция saby_read_changes_get_broker_mark(context)
	last_event = Неопределено;
	
	last_event_id = block_obj_get_path_value(context, "operation.data.public_params.last_event.LastEventEdo.id", "");
	last_event_datetime = block_obj_get_path_value(context, "operation.data.public_params.last_event.LastEventEdo.datetime", "");
	last_document_id = block_obj_get_path_value(context, "operation.data.public_params.last_event.LastEventEdo.doc_id", "");
	
	Если last_event_id = Неопределено И last_event_datetime = Неопределено И last_document_id = Неопределено Тогда //
	...
	Результат.Вставить("last_event", last_event);
	Результат.Вставить("last_event_id", last_event_id);
	Результат.Вставить("last_event_datetime", last_event_datetime);
	Результат.Вставить("last_document_id", last_document_id); 
	Возврат Результат;
КонецФункции
...
Функция saby_read_changes_get_items(context, block_context)
    FILTER = get_prop(block_context, "FILTER", Новый Соответствие);
...
// BSLLS:DeprecatedCurrentDate-on
    ...
    broker_mark = saby_read_changes_get_broker_mark(context);
	
	//Попробуем вычитать точку с подключения
	last_event = get_prop(broker_mark, "last_event");
	last_event_id = get_prop(broker_mark, "last_event_id");
	last_document_id = get_prop(broker_mark, "last_document_id");
	last_event_datetime	= get_prop(broker_mark, "last_event_datetime");
	...
	Для каждого ЭлементФильтра из FILTER Цикл
		_filter.Вставить(ЭлементФильтра.Ключ, ЭлементФильтра.Значение); 
	КонецЦикла;
	...
	//добавляем в ДопПоля ПервичныйКлюч
	extra_fields = get_prop(_filter, "ДопПоля");
	Если Не extra_fields = Неопределено Тогда 
		// BSLLS:IfElseIfEndsWithElse-off
		Если ТипЗнч(extra_fields) = Тип("Массив") Тогда
			Если extra_fields.Найти("ПервичныйКлюч") = Неопределено Тогда
				extra_fields.Добавить("ПервичныйКлюч");
			КонецЕсли;
			_filter["ДопПоля"] = Saby_СтрСоединить82(extra_fields,",");
		ИначеЕсли 
			ТипЗнч(extra_fields) = Тип("Строка") И Найти(extra_fields, "ПервичныйКлюч") = 0 Тогда
			_filter["ДопПоля"] = extra_fields + ",ПервичныйКлюч";
		КонецЕсли;
	...
	LastEventEdo_datetime = block_obj_get_path_value(context, "operation.update_public_params.last_event.LastEventEdo.datetime", Неопределено);	
	// BSLLS:IfElseIfEndsWithElse-off
	Если (Не _filter.Свойство("ДатаВремяС") И Не _filter.Свойство("ИдентификаторСобытия") И Не _filter.Свойство("ИдентификаторДокумента")) 
			Или LastEventEdo_datetime <> Неопределено Тогда
			
		Если last_event_datetime = Неопределено and Не ЗначениеЗаполнено(last_event) Тогда
			...
			block_obj_set_path_value(context,"operation.update_public_params.last_event.LastEventEdo.datetime", last_event_datetime);
		КонецЕсли;
	...
	ИначеЕсли _filter.Свойство("ДатаВремяС") И ЗначениеЗаполнено(_filter["ДатаВремяС"]) Тогда
		last_event_datetime = _filter["ДатаВремяС"];	
	КонецЕсли;
...
Функция saby_read_changes_execute_item(node_loop, path, context, block_context)
	add_param_api3(block_context["items"][block_context["index"]], context);        
	doc = block_context["items"][block_context["index"]];
	event = doc["Событие"][0];
	begin = ДатаВМиллисекундах();
	Title = "Получение списка изменений";
	doc = block_context["items"][block_context["index"]];
	Subtitle = get_prop(doc, "Название");
	event = doc["Событие"][0];
	Data = Новый Структура; 
	message = Новый Структура;
	message.Вставить("ИдентификаторСобытия", get_prop(event, "Идентификатор"));
	message.Вставить("ИдентификаторДокумента", get_prop(doc, "Идентификатор"));
	message.Вставить("Дата", get_prop(doc, "Дата"));
	message.Вставить("Номер", get_prop(doc, "Номер"));
	...
	block_obj_set_path_value(context,"operation.data.public_params.last_event.LastEventEdo.id", event["Идентификатор"]);	
	block_obj_set_path_value(context,"operation.update_public_params.last_event.LastEventEdo.id", event["Идентификатор"]);	

	block_obj_set_path_value(context,"operation.data.public_params.last_event.LastEventEdo.datetime", event["ДатаВремя"]);	
	block_obj_set_path_value(context,"operation.update_public_params.last_event.LastEventEdo.datetime", event["ДатаВремя"]);	

	block_obj_set_path_value(context, "operation.data.public_params.last_event.LastEventEdo.doc_id", doc["Идентификатор"]);	
	block_obj_set_path_value(context, "operation.update_public_params.last_event.LastEventEdo.doc_id", doc["Идентификатор"]);	
