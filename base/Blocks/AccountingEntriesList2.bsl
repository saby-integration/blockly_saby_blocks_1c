...
Функция accounting_entries_list2_items(context, block_context, local_context)
	...
	Пока local_context["Выборка"].СледующийПоЗначениюПоля("Регистратор") Цикл
		...
		Если ВыгружатьПроводки = "TRUE" Тогда
			...
			Если    local_context["Выборка"].Регистратор.Дата < block_context.filter["dateStart"] 
				ИЛИ local_context["Выборка"].Регистратор.Дата > КонецДня(block_context.filter["dateEnd"]) Тогда
				// Запросим все проводки по документу, если его дата не входит в загружаемый период
				ТекущаяВыборка = ВыборкаХозрасчетныхДвиженийССубконто2ПоДокументу(local_context["Выборка"].Регистратор,
									local_context["Выборка"].Организация);
			Иначе
				ТекущаяВыборка = local_context["Выборка"];
			КонецЕсли;
			Пока ТекущаяВыборка.Следующий() Цикл
				// НЕ ГРУЗИМ ПРОВОДКИ ПО ЗАБАЛАНСОВЫМ СЧЕТАМ!!!
				Если НЕ ЗначениеЗаполнено(ТекущаяВыборка.СчетДт) ИЛИ НЕ ЗначениеЗаполнено(ТекущаяВыборка.СчетКт) Тогда
					Продолжить;
				КонецЕсли;
				
				ПроводкаСыраяДебет = Новый Структура("КодСчета,Аналитика1ИС,Аналитика2ИС,Аналитика3ИС", ТекущаяВыборка.СчетДт,ТекущаяВыборка.СубконтоДт1,ТекущаяВыборка.СубконтоДт2,ТекущаяВыборка.СубконтоДт3);
				ПроводкаСыраяКредит = Новый Структура("КодСчета,Аналитика1ИС,Аналитика2ИС,Аналитика3ИС", ТекущаяВыборка.СчетКт,ТекущаяВыборка.СубконтоКт1,ТекущаяВыборка.СубконтоКт2,ТекущаяВыборка.СубконтоКт3);
				ПроводкиСырые.Добавить(Новый Структура("Дебет,Кредит,Сумма,Дата", ПроводкаСыраяДебет, ПроводкаСыраяКредит, ТекущаяВыборка.Сумма, Формат(ТекущаяВыборка.Период, "ДФ=dd.MM.yyyy")));
				...
				Проводки.Добавить(Новый Структура("Дебет,Кредит,Сумма,Дата", ПроводкаДебет, ПроводкаКредит, ТекущаяВыборка.Сумма, Формат(ТекущаяВыборка.Период, "ДФ=dd.MM.yyyy")));
			КонецЦикла;
		...
		ini_name = "Blockly_"+ОбъектМетаданных.Имя + "_read";
		Попытка
			Load_ini(ini_name);
...
Функция ВыборкаХозрасчетныхДвиженийССубконто2ПоДокументу(Регистратор, Организация)
	Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ
        |   ХозрасчетныйДвиженияССубконто.Период КАК Период,
        ...
        |ИЗ
        |   РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(,,
        |       Активность = Истина И Организация = &Организация И Регистратор = &Регистратор
		|   ) КАК ХозрасчетныйДвиженияССубконто
		|ГДЕ
		|   ХозрасчетныйДвиженияССубконто.Сумма <> 0
		|УПОРЯДОЧИТЬ ПО
