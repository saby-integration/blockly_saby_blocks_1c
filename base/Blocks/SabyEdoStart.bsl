...
Функция block_saby_edo_start_write_document_get_value(block_context, context, doc, multithread_mode)
	...
	Если QueryId = Неопределено Тогда  		
		Попытка
			Результат = Транспорт.local_helper_write_document(context.params, doc, multithread_mode, 120);
		Исключение
	...
	Иначе 
		Попытка
			responce = Транспорт.local_helper_exec_method_process_responce_async(context.params, QueryId);
			Результат = Транспорт.local_helper_write_document_process_responce(responce);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			Результат = Ложь;
...
Функция block_saby_edo_start_prepare_result_get_value(block_context, context, execute_action, multithread_mode)
	...
	Если QueryId = Неопределено Тогда  		
		Попытка
			prepare_result = Транспорт.local_helper_prepare_action(context.params, execute_action, multithread_mode, 120);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			Если ЭтоСлужебнаяОшибкаБлока(ОшибкаСтруктура.type) Тогда 
	...
	Иначе 
		Попытка
			responce = Транспорт.local_helper_exec_method_process_responce_async(context.params, QueryId);
			prepare_result = Транспорт.local_helper_prepare_action_process_responce(responce);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			prepare_result = Ложь;
...
Функция block_saby_edo_start_execute_action_get_value(block_context, context, execute_action, multithread_mode)
	...
	Если QueryId = Неопределено Тогда  		
		Попытка
			execute_action = Транспорт.local_helper_execute_action(context.params, execute_action, multithread_mode, 120);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			Если ЭтоСлужебнаяОшибкаБлока(ОшибкаСтруктура.type) Тогда 
	...
	Иначе 
		Попытка
			responce = Транспорт.local_helper_exec_method_process_responce_async(context.params, QueryId);
			execute_action = Транспорт.local_helper_execute_action_process_responce(responce);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ОшибкаСтруктура = NewExtExceptionСтруктура(ИнфОбОшибке);
			execute_action = Ложь;
