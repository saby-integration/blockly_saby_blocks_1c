...
Функция block_compare_objects_calc_value(block_type, node, path, context, block_context)
	...
	Если НЕ ЗначениеЗаполнено(Объект1) И НЕ ЗначениеЗаполнено(Объект2) Тогда
		Возврат "";
	КонецЕсли;
	ТекстОшибки = "";
	...
	// Сравниваем только существующие свойства
	СуществующиеСвойства = Неопределено;
	Если ЗначениеЗаполнено(Свойства) Тогда
		СуществующиеСвойства = Новый Массив;
		Для Каждого Свойство Из Свойства Цикл
			Если НЕ (get_prop(Объект1, Свойство) = Неопределено И get_prop(Объект2, Свойство) = Неопределено) Тогда
				СуществующиеСвойства.Добавить(Свойство);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	// Параметры для сравнения ШАПОК структур/объектов
	// сравниваем ТОЛЬКО УКАЗАННЫЕ СУЩЕСТВУЮЩИЕ свойства, если нет - не сравниваем
	compare_params = Новый Соответствие;
	compare_params.Вставить("PROPS", СуществующиеСвойства);
