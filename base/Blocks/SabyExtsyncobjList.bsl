...
Функция	saby_extsyncobj_list_items(context, block_context)
	...
	filter = Новый Структура("SyncDocId,Subobject", extsyncdoc_uuid, Ложь);
	
	sorting = Новый Массив;
	Страница	= 0;
	Если НЕ block_context.Свойство("page", Страница) Тогда
		Страница = 0;
	КонецЕсли;
	Если Страница = 0 Тогда
		report_write_unregistered_objects(extsyncdoc_params());
	КонецЕсли;
	pagination = Новый Структура("PageSize, Page", 15, Страница);
