...
Функция block_extsyncdoc_write2_calc_value(block_type, node, path, context, block_context)
	...
	result = block_extsyncdoc_write(direction, objects, context, run);
...
Функция block_extsyncdoc_write_api3_link(context, direction, objects, заполнять_data_is = Ложь)
	...
	Для Каждого obj Из objects Цикл
		...
		_id, Title, ТипИмяИС, ТипИмяИС,	_id,	Data, StatusId, StatusMsg);                      
...
Процедура ДобавитьДанныеПоТипуОбъектов(Список, filter)
	...
	Для Каждого Запись Из Список Цикл
		...
		Если get_prop(Запись, "ТипИС", "") = "" Тогда
			МассивТипов = СтрРазделить82(Запись["ИмяИС"],".");
...
Функция СформироватьAPI3СсылкуИзЭлементаСписка(ЭлементСписка)
	...
	API3_ref.Вставить("ИдИС", get_prop(ЭлементСписка, "ИдИС"));
	API3_ref.Вставить("ТипИС", get_prop(ЭлементСписка, "ТипИС"));
	API3_ref.Вставить("ИмяИС", get_prop(ЭлементСписка, "ИмяИС"));
	API3_ref.Вставить("ТипСБИС", get_prop(ЭлементСписка, "ТипСБИС"));
	API3_ref.Вставить("ИмяСБИС", get_prop(ЭлементСписка, "ИмяСБИС"));
	API3_ref.Вставить("ИдСБИС", get_prop(ЭлементСписка, "ИдСБИС"));
