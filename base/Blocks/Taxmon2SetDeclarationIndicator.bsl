...
Функция block_taxmon2_set_declaration_indicator_calc_value(block_type, node, path, context, block_context)
	...
	ДубльСтроки = Ложь;
	Для каждого СтрокаРасшифровки Из Отчет["Данные"] Цикл
		Если get_prop(СтрокаРасшифровки, "Ид") = block_context["NAME"] Тогда
			ДубльСтроки = Истина;
			СтрокаРасшифровки["Сумма"] = СтрокаРасшифровки["Сумма"] + get_prop(result, "Сумма", 0);
			Для каждого ЭлементФормулы Из get_prop(result, "Формула", Новый Массив) Цикл
				СтрокаРасшифровки["Формула"].Добавить(ЭлементФормулы);
			КонецЦикла;
			Продолжить;
		КонецЕсли;		
	КонецЦикла;
	
	Если Не ДубльСтроки Тогда
