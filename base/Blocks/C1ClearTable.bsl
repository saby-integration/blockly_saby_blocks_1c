	...
	Попытка
		#Если Сервер Тогда
			Если Найти(НРег(ТипЗнч(res)), " табличная часть") Или ТипЗнч(res) = Тип("ТаблицаЗначений") 
				Или Найти(НРег(ТипЗнч(res)), " tabular section") Или ТипЗнч(res) = Тип("ValueTable") 
				Или ТипЗнч(res) = Тип("Массив")
				Или ТипЗнч(res) = Тип("Array") Тогда
				res.Очистить();
			Иначе
				Сообщить("Неизвестный блок в методе c1_clear_table");
			КонецЕсли;
		#Иначе 
			Если ТипЗнч(res) = Тип("Массив") 
				Или ТипЗнч(res) = Тип("Array") Тогда
				res.Очистить();
			Иначе
				Сообщить("Неизвестный блок в методе c1_clear_table");
