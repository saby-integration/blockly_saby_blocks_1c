...
Функция ПрикрепленныеФайлыПоТипу(block_context, ТипФайлов = "")
	...
	Для Каждого ПрикрепленныйФайл Из МассивСсылок Цикл
		// Двоичные данные получаем через общий модуль
		ОбщийМодульРаботаСФайлами = ОбщийМодульКонфигурации("РаботаСФайлами");
		Если ОбщийМодульРаботаСФайлами = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ФайлДД      = РаботаСФайлами.ДвоичныеДанныеФайла(ПрикрепленныйФайл.Ссылка);
		ИмяФайла    = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(ПрикрепленныйФайл.Наименование,
						ПрикрепленныйФайл.Расширение);
