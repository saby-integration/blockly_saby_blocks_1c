...
Функция block_test_read_saby_object_calc_value (block_type, node, path, context, block_context)
	...
	resp = Транспорт.local_helper_find_sbis_object(context.params, block_context.type, block_context.keys);
    // если не находим ругаемся
	list_obj = get_prop(resp, "result");
	// добавляем в обмен
	Если list_obj = Неопределено ИЛИ list_obj.Количество() = 0 Тогда
		ВызватьИсключение NewExtExceptionСтрока(,"Тестовый объект не найдет в "+ЛокализацияНазваниеПродукта(),,"block_test_read_saby_object",);
	КонецЕсли;
	obj = list_obj[0];
	// запускаем обмен            
	...
	extsyncobjects.Добавить(Новый Структура("Title, Type, SbisId", get_prop(obj, "Название"), block_context.type, obj_saby_id));
	
	// записываем ссылку
	connection_uuid = get_prop(context.operation,"connection_uuid","");
	extsyncdoc_uuid = Транспорт.local_helper_extsyncdoc_write(
		context.params,
		connection_uuid,
	...
	resp = Транспорт.local_helper_extsyncobj_list(context.params, , filter, , );
